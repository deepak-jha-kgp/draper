...
export default defineConfig(({ mode }) => {
  ...
  // The preview harness assigns a port through PORT and expects the server to
  // land on exactly that one; `--port` in the dev script would silently win and
  // the pane would then watch a port nothing is listening on.
  ...
  const server = proxyTarget
    ...
    : { port: assignedPort, strictPort: Boolean(assignedPort) }
